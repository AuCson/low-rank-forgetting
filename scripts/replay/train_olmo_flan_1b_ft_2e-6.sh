...
# random
accelerate launch --num_processes 2 --main_process_port 29501 run_llm_ocl.py \
--config_files configs/defaults.yaml configs/llm/llm_defaults.yaml \
configs/llm/replay_ocl/1b_ft_1kstep_er.yaml \
--ocl_task ${task_category} --templates TASK_ID=${task_id} TASK_CATEGORY=${task_category} mixture_ratio=${mixture_ratio}

# additive-offline
accelerate launch --num_processes 2 --main_process_port 29501 run_llm_ocl.py \
--config_files configs/defaults.yaml configs/llm/llm_defaults.yaml \
configs/llm/replay_ocl/1b_ft_1kstep_additive${yaml_postfix}.yaml \
...
accelerate launch --num_processes 2 --main_process_port 29501 run_llm_ocl.py \
--config_files configs/defaults.yaml configs/llm/llm_defaults.yaml \
configs/llm/replay_ocl/1b_ft_1kstep_knn${yaml_postfix}.yaml \
--ocl_task ${task_category} --templates TASK_ID=${task_id} TASK_CATEGORY=${task_category} mixture_ratio=${mixture_ratio} mixture_method=pred_sample temperature=${temperature}

# mf-offline
accelerate launch --num_processes 2 --main_process_port 29501 run_llm_ocl.py \
--config_files configs/defaults.yaml configs/llm/llm_defaults.yaml \
configs/llm/replay_ocl/1b_ft_1kstep_svd${yaml_postfix}.yaml \
--ocl_task ${task_category} --templates TASK_ID=${task_id} TASK_CATEGORY=${task_category} mixture_ratio=${mixture_ratio} mixture_method=pred_sample temperature=${temperature}

# ground-truth forgetting
accelerate launch --num_processes 2 --main_process_port 29505 run_llm_ocl.py \
--config_files configs/defaults.yaml configs/llm/llm_defaults.yaml \
configs/llm/replay_ocl/1b_ft_1kstep_gt.yaml \
